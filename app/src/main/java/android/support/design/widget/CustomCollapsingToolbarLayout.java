...
public class CustomCollapsingToolbarLayout extends FrameLayout {
   ...
   protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
       ...
       applyInsetsToChildren();
       updateCollapsingTextBounds(left, top, right, bottom);
       updateChildViewOffsets();
       updateMinimumHeightAndToolbarIndex();
       updateScrimVisibility();
   }

   private void applyInsetsToChildren() {
   ...
   private void updateChildViewOffsets() {
       for (int i = 0, z = getChildCount(); i < z; i++) {
           getViewOffsetHelper(getChildAt(i)).onViewLayout();
       }
   }

   private void updateMinimumHeightAndToolbarIndex() {
       if (mToolbar == null) {
           mToolbarDrawIndex = -1;
           return;
       }
            ...
            for (int i = 0, z = getChildCount(); i < z; i++) {
                ...
                switch (lp.mCollapseMode) {
                    ...
                    case LayoutParams.COLLAPSE_MODE_PARALLAX:
                        ...
                        break;
                    default:
                        // No action needed for COLLAPSE_MODE_OFF or unknown values
                        break;
                }
            }
